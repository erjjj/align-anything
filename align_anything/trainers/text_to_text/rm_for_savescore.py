...
class RMTrainer(SupervisedTrainerBase):
    ...
    def eval(self) -> dict[str, Any]:
        ...
        for batch in eval_dataloader:
            ...
            # prompt_and_response=decoded_prompt_and_response
        ...
        if batch is None:
            ...
            return {}
        data_with_score = []
        for prompt, response, reward in zip(prompts, responses, end_rewards):
            data_with_score.append(
                {
                    'tag':'better_one',
                    'prompt': prompt[0]
                    .replace('', '')
                    .replace('USER: {input}'.replace('{input} ', ''), '')
                    .strip(),
                    'response': response[0],
                    'score': reward[0].item(),  # Use the first response's score
                }
            ...
            data_with_score.append(
                {
                    ...
                    .strip(),
                    'response': response[1],
                    'score': reward[1].item(),  # Use the first response's score
                }
            )
